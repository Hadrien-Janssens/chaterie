...
const CatCard = ({ chaton }) => {
  ...
  // const { img, loading, error } = useFetchImg(chaton.cha_id);
  // chaton.img = img;

  // if (!loading) {
  //   // const [isFetching, setIsFetching ] = useState(false)
  //   console.log(chaton.img[0]?.name);
  //   if (chaton.img) {
  //     setImgKey(chaton.img[0]?.name);
  //   }
  // }
  const { img: fetchedImg, loading, error } = useFetchImg(chaton.cha_id);

  useEffect(() => {
    if (!loading && fetchedImg.length > 0) {
      setImgKey(fetchedImg[0].name);
    }
  }, [loading, fetchedImg]);
      ...
      <div className="flex justify-center gap-10 flex-wrap ">
        <div>
          ...
          <div className=" flex justify-center mt-3 gap-1">
            {!toggle &&
              fetchedImg.map((img, key) => (
                <img
                  src={img.name}
                  key={key}
                  alt=""
                  className="w-10 cursor-pointer"
                  onClick={() => setImgKey(img.name)}
                />
              ))}
          </div>
        </div>
